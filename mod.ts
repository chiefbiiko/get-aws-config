...
function parse(file: string): Doc {
  return dcdr.decode(Deno.readFileSync(file))
    .split(/\r?\n/)
    .map((line: string): string => line.trim())
    .reduce(([oldProfile, acc]: [string, Doc], line: string): [string, Doc] => {
      ...
      if (line.startsWith("[")) {
        ...
        .split("=")
        .map((part: string): string => part.trim().replace(/^["']["']$/, ""))
...
export function get (conf?: GetConfig): Doc {
  const _conf: Doc = {profile: "default",
  sharedCredentialsFile: `${Deno.homeDir()}/.aws/credentials`,
  configFile:  `${Deno.homeDir()}/.aws/config`, env: true, fs: true, ...conf }
  ...
  if (_conf.fs) {
    ...
    const credsFile: string = conf.sharedCredentialsFile || env.AWS_SHARED_CREDENTIALS_FILE || _conf.sharedCredentialsFile
    const configFile: string = conf.configFile || env.AWS_CONFIG_FILE || _conf.configFile
    ...
    return {
      ...config[profile], 
      ...creds[profile],
      accessKeyId: env.AWS_ACCESS_KEY_ID || creds[profile].accessKeyId || config[profile].accessKeyId,
      secretAccessKey: env.AWS_SECRET_ACCESS_KEY || creds[profile].secretAccessKey || config[profile].secretAccessKey,
      sessionToken: env.AWS_SESSION_TOKEN || creds[profile].sessionToken || config[profile].sessionToken,
      region: env.AWS_DEFAULT_REGION || creds[profile].region || config[profile].region
    }
  }
